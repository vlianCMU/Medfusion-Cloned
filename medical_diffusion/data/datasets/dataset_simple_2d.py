...
class FundusControlNetDataset(SimpleDataset2D):
    ...
    def _load_control(self, stem: str):
        """
        将 vessel 和 disc_cup 合成为一张 3 通道的图像:
        - R 通道: vessel (血管)
        - G 通道: disc (视盘)
        - B 通道: cup (视杯)
        """
        ...
        vessel_img = Image.open(vessel_path).convert('RGB')
        vessel_array = np.array(vessel_img)
        # vessel 的三个通道值相同,只取 R 通道即可
        vessel_channel = Image.fromarray(vessel_array[:, :, 0])
        
        if self.disc_cup_dir is not None:
            # 加载 disc_cup 图像
            disc_cup_path = self.disc_cup_dir / f"{stem}.{self.control_ext}"
            disc_cup_img = Image.open(disc_cup_path).convert('RGB')
            disc_cup_array = np.array(disc_cup_img)
            
            # 提取 disc (R 通道) 和 cup (B 通道)
            disc_channel = Image.fromarray(disc_cup_array[:, :, 0])  # R 通道 = 视盘
            cup_channel = Image.fromarray(disc_cup_array[:, :, 2])   # B 通道 = 视杯
            
            # 合成为 RGB: R=vessel, G=disc, B=cup
            control_rgb = Image.merge('RGB', (vessel_channel, disc_channel, cup_channel))
        else:
            # 如果没有 disc_cup,只使用 vessel
            control_rgb = Image.merge('RGB', (
                vessel_channel, 
                Image.new('L', vessel_channel.size, 0),
                Image.new('L', vessel_channel.size, 0)
            ))
        
        # 应用 transform
        control_tensor = self.control_transform(control_rgb)
