...
class APTOSDataset(Dataset):
    def __init__(self, split_dir, img_size=256):
        ...
        for c in CLS_NAMES:
            ...
            for p in sorted(cdir.glob("*.jpg")): ## 这里可能要改
...
class UNetBottleneckClassifier(nn.Module):
    ...
    def forward(self, x):
        ...
        for t_val in self.t_steps:
            ...
            gap = F.adaptive_avg_pool2d(feat256, (1,1)).squeeze(-1).squeeze(-1)
            gap = F.layer_norm(gap, (gap.size(-1),))
            vecs.append(gap)
...
def main():
    ...
    ap.add_argument("--mode", type=str, choices=["freeze","finetune"], default="freeze",
    ...
    ap.add_argument("--ckpt", type=str, default="/data1/lhy/medfusion-main/runs/2025_09_17_081313无公开数据集正常不剔除/lightning_logs/version_0/checkpoints/last.ckpt")
    ...
    ap.add_argument("--t_steps", type=int, nargs=2, default=[50, 750])
    ap.add_argument("--out_dir", type=str, default="/data1/lhy/medfusion-main/classification")
