...
from condition_adaptor_src.condition_adaptor_dataset import T2ICollectedDataset, U2ICollectedDataset
...
class ConditionAdaptorRunner:
    def __init__(self, rank, configs):
        ...
        self.diffusion_steps = self.model_configs['model']['params']['timesteps']  #1000 T
        self.blocks = self.model_configs['condition_adaptor_config']['blocks']   #[2,4,8];[2,4,8,12] F
        
        self.diffusion_model = instantiate_from_config(self.model_configs['model'])  #在这里是training True 
        self.diffusion_model.eval().cuda(self.global_rank)   #在这里就是training False，后面都遍历一遍加上no grad试一下
    ...
    def evaluate_t2i(self):
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(self.val_loader):
                ...
                cond = self.vq_model.encode_to_codebook(cond)   #都是靠vae提特征
                ...
                # forward
                cond_pred = self.model(upsampled_features, t)  #和train一模一样的过程
                ...
                if i % self.model_configs['condition_adaptor_config']['val_sample_freq'] == 0:
                    ...
                    image_sample = self.vq_model.decode(image_sample)  #decode原来的image，没有问题
                    ...
                    # visualize
                    self.visualize(item_dict=item_dict, epoch=self.epoch)
    ...
    def T2I_training(self):
        ...
        for epoch in range(args.epochs):
            ...
            for i, batch in enumerate(train_loader):
                ...
                # TODO: foward vector quantatize model & text condition model (CLIP)
                image = self.vq_model.encode_to_codebook(image)  #grad none
                cond = self.vq_model.encode_to_codebook(cond)   #grad none
                
                self.cond_model = self.cond_model.cuda(0)
                text_cond = self.cond_model(text).cuda(self.global_rank)  #grad none
                ...
                noisy_img = self.diffusion_model.q_sample(x_start=image, t=t, noise=noise)
                features = self.diffusion_model.model.diffusion_model.forward_return_features(noisy_img, t, text_cond, block_indexes=self.blocks)
                #此时无grad,但有grad_fn，因为diffusion model没完全禁止grad，不确定diffusion_model有没有被更新
                upsampled_features = self.upsample_features(size=CA_configs['size'], features=features)
                
                self.optimizer.zero_grad()
                
                # forward
                cond_pred = self.model(upsampled_features, t)  #输出的是预测出来的mask
                
                # update
                # loss = self.criterion(cond, cond_pred)
                loss = train_criterion(cond,cond_pred) *100
                ...
                if self.iteration > args.max_steps:
                    ...
                    torch.save({'model_state_dict': self.model.state_dict()}, save_path)  #只存了model的参数，说明fiffusion没有被更新
                    cleanup()
                    sys.exit(0)
                # break
    ...
    def U2I_training(self):
        ...
        for epoch in range(args.epochs):
            for i, batch in enumerate(train_loader):
                ...
                # TODO: forward diffusion model to extract the internal representations
                t = torch.randint(0, self.diffusion_steps, (image.size(0),), device=f'cuda:{self.global_rank}').long()
                noisy_img = self.diffusion_model.q_sample(x_start=image, t=t, noise=noise) #从这个Image开始采样
    ...
    def visualize(self, item_dict, epoch):
        ...
        for key in item_dict.keys():
            ...
            final_dict[key] = concat_tensor  #三模态都存在
        ...
        with torch.no_grad():
            with diffusion_model.ema_scope():
                ...
                if CA_configs['cond_type'] == 'edge':
                    _, target_cond = cv2.threshold(target_cond, 127.5, 255.0, cv2.THRESH_BINARY)# 如果是edge需要处理这一步
                ...
                target_cond = vq_model.encode_to_codebook(target_cond)     #这时候没有图像了           
                
                # TODO: forward text condition model - CLIP
                text_cond = cond_stage_model(self.args.caption)   #文本特征             
                ...
                outputs = sampler.sample(S=self.args.steps,
                                        ...
                                        truncation_steps=self.args.truncation_steps,)  #生成过程，他把model融入了采样过程
                
                # TODO: unpack returned outputs
                samples_ddim, inter = outputs
    ...
    def U2I_sampling_single_image_with_CA(self, args, input_path, cond_fn):
        ...
        with torch.no_grad():
            with diffusion_model.ema_scope():
                ...
                target_cond = cv2.imread(input_path)    #只用了mask？
