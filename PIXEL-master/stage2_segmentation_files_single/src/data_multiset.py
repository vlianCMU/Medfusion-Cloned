...
class LungDataset(torch.utils.data.Dataset):
    def __init__(self, origin_mask_list, origins_folder,origins_folder2, masks_folder, masks_folder2, transforms=None):
    ...
    def __getitem__(self, idx):
        origin_name = self.origin_mask_list[idx]
        origin = Image.open(self.origins_folder / (origin_name + ".jpg")).convert("P")
        mask = Image.open(self.masks_folder / (origin_name + ".jpg")).convert("P")
        ...
        mask = np.array(mask)
        mask = (torch.tensor(mask) > 128).long() #归一化，如果不是二值不需要有这一步
        return origin, mask, origin_name+'.jpg'
